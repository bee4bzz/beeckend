...
async def test_create_item(async_get_db: AsyncSession) -> None:
    item_crud = crud.CRUDItem(async_get_db, models.Item)
    title = random_lower_string()
    description = random_lower_string()
    item_in = ItemCreate(title=title, description=description)
    user = await create_random_user(async_get_db)
    item = await item_crud.create_with_owner(create_dict=item_in, owner_id=user.id)
...
async def test_get_item(async_get_db: AsyncSession) -> None:
    ...
    user = await create_random_user(async_get_db)
    item = await item_crud.create_with_owner(create_dict=item_in, owner_id=user.id)
    stored_item = await item_crud.get(id=item.id)
    assert stored_item
...
async def test_update_item(async_get_db: AsyncSession) -> None:
    ...
    item2 = await item_crud.update(db_obj=item, obj_in=item_update)
